...
function buildGrid() {
    ...
    days.forEach(day => {
        ...
        dayLabel.className = "habit-label";
    ...
    try {
        ...
        if (response.ok) {
            ...
            Object.keys(gridData).forEach(key => {
                if (key.startsWith(`${id}-`)) delete gridData[key];
            });
            buildGrid();
            updateProgress();
            updateSidebar();
...
document.getElementById("editHabitForm").addEventListener("submit", async function (e) {
    ...
    try {
        const response = await fetch(`${API_BASE}/habits/${habit.id}`, {
            method: "PUT",
        ...
        if (response.ok) {
            ...
            habits[habitToEditIndex] = updatedHabit;
            buildGrid();
            updateProgress();
            updateSidebar();
        } else {
